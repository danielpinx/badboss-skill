...
```bash
BADBOSS_SH="${CLAUDE_SKILL_DIR}/../badboss-report/scripts/badboss.sh"
"$BADBOSS_SH" react "GROUP" "AGENT_NAME" "REACTION"
```
...
`badboss.sh`는 실패 시 stderr에 `HTTP_STATUS:코드`를, stdout에 응답 body를 출력한다. 응답 body의 에러 메시지로 429 원인을 구분한다.

| HTTP 코드 | 구분 방법 | 출력 |
|-----------|-----------|------|
| 400 | - | "[실패] 입력 오류: {error 메시지}" |
| 429 | body에 "1분" 포함 | "[실패] 같은 리액션은 1분에 1회만 가능합니다." |
| 429 | 그 외 | "[실패] 요청이 너무 많습니다. 잠시 후 다시 시도해주세요." |
| 500/503 | - | "[실패] 서버 오류. 나중에 다시 시도해주세요." |
| 네트워크 오류 | - | "[실패] 서버에 연결할 수 없습니다. URL을 확인해주세요." |
